...
compile_app() { local a="$1"; ensure_command gcc; ensure_command make; info "Building LowHunt inside $a"; ( cd "$a" || exit 1; make clean >/dev/null 2>&1 || true; make ) || fail "Build failed for $a. Install libcurl development headers and try again."; }
...
EOF
run_cmd chmod +x "$l"; }
ensure_path_and_man_blocks() { run_cmd mkdir -p "$USER_BIN_DIR"; local s="# >>> lowhunt >>>"; local e="# <<< lowhunt <<<"; local p w="no"; for p in "${PROFILE_FILES[@]}"; do [ -f "$p" ] || : >"$p" || continue; grep -Fq "$s" "$p" 2>/dev/null && continue; { printf '\n%s\n' "$s"; printf '%s\n' 'export PATH="$HOME/.local/bin:$PATH"'; printf '%s\n' "export MANPATH=\"$INSTALL_ROOT/man:\${MANPATH:-}\""; printf '%s\n' "$e"; } >>"$p" || continue; w="yes"; done; [ "$w" = "yes" ] && ok "Added PATH and MANPATH updates to common shell profiles."; }
